...
contract TestBalancerLiquidityPoolAbstraction is Env
{
	function test01() public
	{
		...
		_mint(MTC, 10e18);
		_mint(DAI, 100e18);
		Assert.equal(_getBalance(MTC), 10e18, "MTC balance must be 10e18");
		Assert.equal(_getBalance(DAI), 100e18, "DAI balance must be 100e18");

		address _pool = BalancerLiquidityPoolAbstraction._createPool(MTC, 10e18, DAI, 100e18);

		Assert.equal(_getBalance(MTC), 0e18, "MTC balance must be 0e18");
		...
		Assert.equal(_getBalance(MTC), 833015029829494720, "MTC balances must be 833015029829494720");
	...
	function test04() public
	{
		...
		_mint(MTC, 1e6);
		_mint(DAI, 1e6);
		Assert.equal(_getBalance(MTC), 1e6, "MTC balance must be 1e6");
		Assert.equal(_getBalance(DAI), 1e6, "DAI balance must be 1e6");

		address _pool = BalancerLiquidityPoolAbstraction._createPool(MTC, 1e6, DAI, 1e6);
